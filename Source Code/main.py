...
def delta_probability_strike_price_finder(tickers_under_price, exp_date, call_or_put):
    ...
    for ticker in tickers_under_price:
        try:
            ...
            while True:
                try:
                    option_data = r.find_options_by_expiration_and_strike(ticker, exp_date, strike_price, call_or_put)
                    if call_or_put == 'put':
                    ...
                    if call_or_put == 'call':
                        if (float(strike_price) % 0.5) == 0:
                            strike_price = str(round(float(strike_price) + 0.50, 1))
                            if float(strike_price) >= float((5*float(tickers_under_price.get(ticker)))):
                                break
                            if float(option_data[0]['delta']) <= 0.15:
                                break
                        else:
                    ...
                    continue
            print(option_data)
            delta_and_win_percent[ticker] = [option_data[0]['last_trade_price'], option_data[0]['delta'], option_data[0]['strike_price'], tickers_under_price[ticker]]
...
def print_option_data(delta_and_win_percent):
    for stock in delta_and_win_percent:
        percent_win = round((float(delta_and_win_percent[stock][0])/float(delta_and_win_percent[stock][2]))*100, 2)
        ...
        print(str(stock) + ":  Percent Win - "
        + str(percent_win) + "% Probability Lose - "
        + str(probability) + "% " + " Strike - " + str(strike)
        + " Option Price - " + str(option_price)
        + " Price - " + str(price))

login('credentials')
get_stocks_or_pull_stocks = input("Would you like to pull stocks from a file or use Robinhood's top 100? (1 for pull from file 2 for top 100)")
...
price_to_filter_by = input("What is the maximum share price for the stocks you are looking for?")
filtered_stocks = filter_stocks_by_price(list_of_stocks, float(price_to_filter_by))
strike_to_filter_by = input("What is the date of expiry of the options you are looking for? (follow the format YYYY-MM-DD)")
calls_or_puts = input("Are you looking for calls or puts? (1 for calls 2 for puts)")
if calls_or_puts == '1':
    options_data = delta_probability_strike_price_finder(filtered_stocks, strike_to_filter_by, 'call')
elif calls_or_puts == '2':
    options_data = delta_probability_strike_price_finder(filtered_stocks, strike_to_filter_by, 'put')
else:
    print("An Error Has Occurred")
